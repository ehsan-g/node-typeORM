...
describe('/custodian/transaction', () => {
  it('creates, signs and sends a transaction', async () => {
    ...
    const submitTxResult = await updateTransaction(
      createTxResult.id,
      'submitted',
    )
    ...
    transactionIdsToCleanUp.push(createTxResult)
  })
  ...
  it('does not encounter an error if you sign two transactions', async () => {
    ...
    const createTxResult1 = await createTransaction(
      addressToSendTo,
      accountIdToSendFrom,
    )

    // // Create the second transaction
    const createTxResult2 = await createTransaction(
      addressToSendTo,
      accountIdToSendFrom,
    )
    ...
    const submitTxResult1 = await updateTransaction(
      createTxResult1.id,
      'submitted',
    )

    // Now submit the second transaction. This is where the bug is encountered
    const submitTxResult2 = await updateTransaction(
      createTxResult2.id,
      'submitted',
    )

    expect(parseInt(signTxResult2.nonce)).toEqual(
      parseInt(signTxResult1.nonce) + 1,
    )
    expect(submitTxResult1.transactionStatus).toEqual('submitted')
    expect(submitTxResult1.transactionHash).toMatch(/^0x.*/)
    expect(submitTxResult2.transactionStatus).toEqual('submitted')
    expect(submitTxResult2.transactionHash).toMatch(/^0x.*/)
